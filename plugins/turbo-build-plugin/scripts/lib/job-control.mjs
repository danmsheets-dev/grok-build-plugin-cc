...
function matchJobReference(jobs, reference, predicate = () => true) {
  ...
  throw new Error(`No run found for "${reference}". Run /turbo-build-plugin:runs to list known runs.`);
}
...
export function buildSingleJobSnapshot(cwd, reference, options = {}) {
  ...
  if (!selected) {
    throw new Error(`No run found for "${reference}". Run /turbo-build-plugin:runs to inspect known runs.`);
  }
...
export function resolveResultJob(cwd, reference) {
  ...
  if (active) {
    throw new Error(`Run ${active.id} is still ${active.status}. Check /turbo-build-plugin:runs and try again once it finishes.`);
  }

  if (reference) {
    throw new Error(`No finished run found for "${reference}". Run /turbo-build-plugin:runs to inspect active runs.`);
  }
...
export function resolveCancelableJob(cwd, reference, options = {}) {
  ...
  if (sessionScopedActiveJobs.length > 1) {
    throw new Error("Multiple Grok Build runs are active. Pass a run id to /turbo-build-plugin:stop.");
  }
