...
- Default to `--model grok-4.6 --effort xhigh` (Extra High Effort) unless the user explicitly requests a different model or effort (`none|minimal|low|medium|high|xhigh|max|ultra`; unknown values are passed through to the CLI).
